const articleActions = (function () {
  ...
  const ARTICLE_LIST_NODE = document.querySelector('.article-list');
  const ADD_ARTICLE_NODE = document.querySelector('.fa-pencil-square-o');
  const SIGN_IN = document.querySelector('.fa-sign-in');
  const LOGOUT_LINK = document.querySelector('.logout-link');
  const HOME = document.querySelector('.home');
  const SHOW_FILTER = document.querySelector('#btn-show-filter');
  ...
  function handleDisplayArticleList(flag) {
    const params = pagination.save();

    articleModel.getArticles(params)
      .then((articles) => {
  ...
  function handleEditArticleAction(article) {
    ...
    articleModel.getArticleById(article.dataset.id)
      .then(article => modalBox.openAndFillInputFields(article))
  ...
  function handleReadMoreClick(articleDomInstance) {
    articleModel.getArticleById(articleDomInstance.dataset.id)
      .then((article) => {
        articleRenderer.clear();
  ...
  function handleLogoutClick() {
    ...
    userModel.logout()
      .then(articleRenderer.showBtnChevron)
      .catch(error => console.log(error));
  }
  ...
  function handleRemoveArticleAction(articleNodeToDelete) {
    articleModel.removeArticleById(articleNodeToDelete.dataset.id)
      .then(article => articleRenderer.removeArticle(articleNodeToDelete))
      .catch(error => console.log(error));
  }

  function addArticle(article) {
    articleModel.addArticle(article)
      .then(article => articleRenderer.renderArticle(article))
      .catch(error => console.log(error));
  }

  function editArticle(article) {
    articleModel.editArticle(article)
      .then(article => articleRenderer.updateArticle(article))
      .catch(error => console.log(error));
  }
